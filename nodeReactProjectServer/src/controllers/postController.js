...
exports.getPosts = async (req, res) => {
  try {
    ...
    const postQuery =
      "SELECT id, title, content, author, attach file, date_format(created_at, '%Y-%m-%d') as created_at FROM posts ORDER BY id DESC";
...
exports.deletePost = async (req, res) => {
  ...
  try {
    ...
    if (post.attach && fs.existsSync(post.attach)) {
      fs.unlinkSync(post.attach); //동기 방식으로 파일을 삭제하는 함수 비동기방식은 fs.unlink()
    }
    return res
      .status(200)
